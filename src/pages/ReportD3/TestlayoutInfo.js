...
import surgurulogo from "../../assets/images/surguru-logo-transparent.png"
            ...
            <div className="right-section"   style={{ marginRight : `${isFullScreen  ?' 30px' :'' }`}}>
                ...
                {!isUndefined(item.name) && item.name !== "" && (
                    <>
                        ...
                        {!isFullScreen &&
                            <div className="edit-button ms-1" style={{ border: item?.data !== undefined ? '1px solid red' : '1px solid lightgrey' }}
...
const TestLayoutInfo = () => {
    ...
    const [ zoneleveloptions , setZoneLevelOptions ] = useState([ ]);
    const [ arealvlOPtions , setAreaLvlevelOptions ] = useState([ ]);
    const [ territoryLevelOptions , setTerritoryLevelOptions ] = useState([ ]);
    ...
    const [sidePanelStates, setSidePanelStates] = useState({});

    const [openPanelId, setOpenPanelId] = useState(null); // <-- this is new
    ...
    useEffect(() => {
        ...
        if (reportSlice.endDate) {
            ...
            const fetchZoneLevelOptions = async () => {
                try {
                  const response = await urlSocket.post('report/retrive-all-child', {});
                  console.log('ZoneLevelOptions :>> ', response.data.data);
                  setZoneLevelOptions(response.data.data); // if you're storing it

                    let arr = []
                    response.data.data.forEach((item) => {
                        if (Array.isArray(item.areas)) {
                          arr = arr.concat(item.areas);
                        }
                      });
                    setAreaLvlevelOptions(arr)


                    let terrArr = []
                    arr.forEach((item) => {
                        if (Array.isArray(item.territories)) {
                          terrArr = terrArr.concat(item.territories);
                          console.log('terrArr :>> ', terrArr);
                        }   
                        });
                    setTerritoryLevelOptions(terrArr)

                } catch (error) {
                    console.error('Error fetching ZoneLevelOptions:', error);
                }
              };
            
              fetchZoneLevelOptions();
            ...
            try {
                dispatch(retrivePageLayout(authInfo, start_date, end_date, false));
            } catch (error) {
        ...
        if (layout !== undefined) {
            await reportSlice.layoutInfo.map(async (ele, idx) => {
                ...
                if (ele.imgSrc !== undefined && ele.imgSrc !== null) {
                    ...
                    clonedItem.relationships = ele?.relationships !== undefined ? ele.relationships : undefined
            ...
            if (result.isConfirmed) {
                if (iconName === 'rectangle_card') {
                    ...
                    updatedLayout[index] = {
                        ...
                        relationships: layoutItem.relationships
                    };
                }
  ...
  console.log("Edit items!" , item);
            if (item.i) {
                ...
                setSidePanelStates(prev => ({
                  ...Object.fromEntries(Object.keys(prev).map(k => [k, false])), // Reset all to false
                  [item.i]: true
                }));
                ...
                // sessionStorage.setItem("blockdata", JSON.stringify(sanitizedItem));
                // sessionStorage.setItem("blockIdx", JSON.stringify(index1));
                setOpenPanelId(item.i); // set active ID
                setSidePanelOpen(true)
              }
                ...
                layout.forEach((item, pos) => {
                    if (ele.i === item.i) {
                        ...
                        if (ele.imgSrc !== undefined && ele.imgSrc !== null) {
                            ...
                            updatedItem.relationships = ele?.relationships !== undefined ? ele.relationships : undefined;
    ...
    const handleCheckboxChange = async (event, name) => {
        ...
        try {
            ...
            await dispatch(retrivePageLayout(authInfo, reportSlice.startDate, reportSlice.endDate, true));
        } catch (error) {
    ...
    const handleZoneLevelChange = async (event) => {
        ...
        console.log('zoneleveloptions  3155:>> ', zoneleveloptions);


        zoneleveloptions.map((item, index) => {
            if (item.parent === selectedValue.parent) {
                console.log('item :>> ', item);
                setAreaLvlevelOptions(item.areas)
    ...
    const handleAreaLevelChange = async (event) => {
        ...
        console.log('arealvlOPtions  3178:>> ',  arealvlOPtions);


        arealvlOPtions.map((item, index) => {
            if (item.parent === selectedValue.parent) {
                console.log('item :>> ', item);
                setTerritoryLevelOptions(selectedValue.territories)
                setselectedTerritory(null)
    ...
    const handleTerritoryLevelChange = async (event) => {
    ...
    return (
        <React.Fragment>
            <div className="page-content" >
                <div>
                    <Breadcrumbs
                        title={`${pageInfo?.name} / ${treeDataSlice?.pageNodeInfo?.title}`}
                        // title={`${reportSlice.selectedReportTemplateInfo?.name} / ${treeDataSlice?.pageNodeInfo.title}`}
                        isBackButtonEnable={true}
                        labelName={'Back'}
                        gotoBack={() => {
                            navigate('/page_tree'); tToggle(1); dispatch(resetState());
                        }}
                        // enableButton={true}
                                    ...
                                    <div className="layout-container">
                                        ...
                                        <div className="chart-content w-100 scroll-design" style={{ zoom: 0.9, borderLeft: '1px solid lightgrey' }}>
                                            <Row>
                                                <Col>
                                                {
                                                    console.log('zoneleveloptions 3425:>> ', zoneleveloptions , arealvlOPtions , territoryLevelOptions)
                                                }
                                                <div style={{ position: 'sticky', top: 0, zIndex: 3, backgroundColor: 'aliceblue', padding: '5px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }} className="d-flex justify-content-between">
                                                    ...
                                                    <div style={{ minWidth: '250px' }}>
                                                        <Label>Select Zone Level</Label>
                                                        <Select classNamePrefix=" w-100"
                                                            options={zoneleveloptions}
                                                            onChange={(e) => handleZoneLevelChange(e)}
                                                            getOptionLabel={(option) => option.hlevel_name}
                                                        />
                                                    </div>
                                                ...
                                                <div style={{ position: 'sticky', top: 0, zIndex: 3, backgroundColor: 'aliceblue', padding: '5px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }} className="d-flex justify-content-between">
                                                    ...
                                                    <div style={{ minWidth: '250px' }}>
                                                        <Label>Select Area Level</Label>
                                                        <Select classNamePrefix=" w-100"
                                                            options={arealvlOPtions}
                                                            onChange={(e) => handleAreaLevelChange(e)}
                                                            value={selectedArea}
                                                            getOptionLabel={(option) => option.hlevel_name}
                                                        />
                                                    </div>
                                                ...
                                                <div style={{ position: 'sticky', top: 0, zIndex: 3, backgroundColor: 'aliceblue', padding: '5px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }} className="d-flex justify-content-between">
                                                    ...
                                                    <div style={{ minWidth: '250px' }}>
                                                        <Label>Select Territory Level</Label>
                                                        <Select classNamePrefix=" w-100"
                                                            options={territoryLevelOptions}
                                                            value={selectedTerritory} // controlled value
                                                            onChange={(e) => handleTerritoryLevelChange(e)}
                                                            getOptionLabel={(option) => option.hlevel_name}
                                                        />
                                                    </div>
                                                            ...
                                                            <div className="chart-container mb-1" style={{ backgroundColor :'aliceblue'}}>
                                                                <div>
                                                                    <Row id="my-specific-row" >
                                                                        {
                                                                            console.log("isSidePanelOpen!!!!!", isSidePanelOpen , sidepanelData)
                                                                        }
                                                                        <Col>
                                                                            {
                                                                                isSidePanelOpen ?
...
{console.log('openPanelId :>> ', openPanelId)
}
...
{openPanelId && (
  ...
  />
                                                                            ...
                                                                            {/* {
                                                                                isSidePanelOpen ?
                                                                                    <>
                                                                                        {Object.entries(sidePanelStates).map(([id, isOpen]) =>
                                                                            ...
                                                                            {/* {
                                                                                isSidePanelOpen &&
                                                                                <>
                                                                                    {Object.entries(sidePanelStates).some(([_, v]) => v) && (
                                                                                        <SidePanel
                                                                                            overlay={true}
                                                                                            updateLayout={(data, rerender) => { updateLayout(data, rerender) }}
                                                                                            layout={layout}
