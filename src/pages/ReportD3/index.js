...
const ReportTemplate = () => {
    console.log('canEdit Report :>> ', canEdit);
...
console.log(reportDB , 'reportDB')
    console.log('authUser 5555555', authUser)
    ...
    const dbInfo = useSelector((state) => state.auth)?.db_info
        ...
        console.log('dbInfo  Redux store  Main:>> ', dbInfo);
...
console.log('reportTemplateList :>> ', reportTemplateList);
    ...
    useEffect(() => {
        ...
        if (authUser?.role === 'User') {
            fetchuserReport(authUser, reportDB)
        }
        else {
            if (dbInfo) {
                fetchData();
...
//     }, []); // Debounce delay to prevent excessive API calls
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.log('dbInfo 127 :>> ', dbInfo);
            var response = await dispatch(getReportTemplateList(authInfo , dbInfo))
    ...
    //         console.log("authUser", authUser, "editInfo", editInfo, "authInfo", authInfo);
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <MetaTags>
                    {/* <title> {`${authUser?.role !== undefined ? authUser?.role : ''}`}| Report Templates</title> */}
                </MetaTags>
                <Container fluid>
...
{
    console.log('authUser 596 :>> ', authUser)
}
                    <Row>
                        {
