...
import {
    ...
    retriveClnKeys, setqueryFilter, retrivePageLayout, setupdatedSliceData, updateSliceFilter, FilterStartDate , FilterEndtDate , resetState , retriveSlicerLayout
...
const LayoutHeader = ({
    ...
    const default_stack_chart_values = [
        { year: 2010, value1: 20, value2: 10, value3: 30 },
        { year: 2011, value1: 40, value2: 20, value3: 50 },
        { year: 2012, value1: 10, value2: 30, value3: 20 },
        { year: 2013, value1: 60, value2: 40, value3: 10 },
        { year: 2014, value1: 30, value2: 50, value3: 40 },
        { year: 2015, value1: 50, value2: 30, value3: 60 },
    ];
    ...
    function getChartKeys(chartName, item, yearKey = 'year', excludeKey = '_id') {
        const chartData = (item?.data?.length > 0 ? item.data[0][item.math_calc] : defaultValues[chartName]) || [];
    ...
    const handleChartSorting = async (data, item) => {
        ...
        var group_Data;
        if (item.data) {
            group_Data = await groupData(item.data[0][item.math_calc])
                                    ...
                                    <div className="chart-dropdown">
                                        <UncontrolledDropdown>
                                            ...
                                            <DropdownMenu className="dropdown-menu-end">
                                                ...
                                                {(() => {
                                                    const chartData = (item?.data?.length > 0 ? item.data[0][item.math_calc] : defaultValues[item.name]) || [];
...
const LayoutInfo = () => {
    ...
    const requestInfo = {
        dbInfo: store.getState()?.auth?.db_info,
        layoutId: layoutId,
        pageInfo: pageInfo,
        userInfo: store.getState()?.auth?.user_data
    }
    var getBreakPoints = {}
...
updateLayoutOnResize()
    ...
    const handleDrop = async (e, item) => {
        console.log('layout handleDrop:>> ', layout);
    ...
    async function showTableFunc(e, item, indx, lay) {
        ...
        dispatch(updateLayoutInfo(rersized !== undefined ? rersized : updtData, requestInfo));
    }   
    ...
    const handleCheckboxChange = async (event, name) => {
        ...
        //  try {
        //     await dispatch(updateSliceFilter(updatedSlicerLayout, requestInfo));
        //     // await  dispatch(retriveSlicerLayout(reportSlice.startDate, reportSlice.endDate, true))

        //     // console.log('reportSlice.startDate :>> ', reportSlice.startDate, reportSlice.endDate);
        //     await dispatch(retrivePageLayout(reportSlice.startDate, reportSlice.endDate, true));
        // } catch (error) {
        ...
        try {
            ...
            dispatch(retriveSlicerLayout(reportSlice.startDate, reportSlice.endDate,  true));
    ...
    const getXaxisValue = async () => {
        ...
        var response = await dispatch(retriveClnKeys(pageNodeInfo.selected_cln_name));
    ...
    try {
        ...
        // console.log("Finally");
        // dispatch(retriveSlicerLayout(start_date, end_date, false))
        dispatch(retrivePageLayout(start_date, end_date, false));
            ...
            <div className="page-content" >
                <div>
                    <Breadcrumbs 
                        ...
                        labelName={'Back'}
                        gotoBack={() => { navigate(-1); tToggle(1); dispatch(resetState());
                        }}
                        // enableButton={true}
                                    ...
                                    <div>
                                            ...
                                            
                                        {
                                            !(!startDate || !endDate) ?
                                                ...
                                                (
                                                    <>
                                                    <div className="sidebar primary-sidebar scroll-design" style={{zoom: 0.8}}>
                                            <div className="sidebar-menu-options ">
                                                <div className={`menu-item ${selectMenu === '1' ? 'active' : ''} sidebar-menu-icon`} draggable onClick={() => { setGrid(!grid); setcharts(false); setSelectMenu('1'); }} >
                                                    <i className="bx bx-grid sidebar-menu-icon-style" />
                                                    <h6>Grid</h6>
                                                </div>

                                                <div className={`menu-item ${selectMenu === '3' ? 'active' : ''} sidebar-menu-icon`} draggable onClick={() => { setcharts(!charts); setGrid(false); setSelectMenu('3'); }}>
                                                    <i className="bx bx-bar-chart-square sidebar-menu-icon-style" />
                                                    <h6>Charts</h6>
                                                </div>

                                                <div className={`menu-item ${selectMenu === '5' ? 'active' : ''} sidebar-menu-icon`} draggable onClick={() => { getXaxisValue(); setSelectMenu('5');   }}>
                                            ...
                                            {selectMenu === '1' && (
                                                <>
                                                    <div style={{ position: 'sticky', top: 0, zIndex: 3, backgroundColor: 'white', padding: '5px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)' }}>
                                                        <Label className="text-primary fw-bold mb-0 p-1"> Click to create </Label>
                                                    </div>
                                                    <div className="layout-options text-center" style={{borderRight: '1px solid #e9e9e9'}}>

                                                        <img src={one_row} alt="One Row" draggable onClick={() => dispatch(createLayout('1'))} />
                                                        <img src={two_col} alt="Two Col" draggable onClick={() => dispatch(createLayout('2'))} />
                                                        <img src={three_col} alt="Three Col" draggable onClick={() => dispatch(createLayout('3'))} />
                                                    </div>
                                                </>
                                                        ...
                                                        <>
                                                            <div className="chart-container mb-1">
                                                                <div>
                                                                   ...
                                                                   {
                                                                    console.log('layout 2992 :>> ', layout , reportSlice.noLayoutdata , reportSlice.endDate , 'isFullScreen' , isFullScreen) 
                                                                   }
                                                                    {
                                                                                                                    ...
                                                                                                                    item.name === 'pie_chart' ?
                                                                                                                        ...
                                                                                                                        item.name === 'line_chart' ?
                                                                                                                            ...
                                                                                                                            : item.name === "rectangle_card" ?
                                                                                                                                ...
                                                                                                                                //     </span>
                                                                                                                                //     <h6 className="mb-0" style={getStyle(item, '2')}>{(item?.filteredcount ? item.filteredcount : (item.count !== null && item.count !== undefined) ? item.count : 'IN- VALID')}</h6>
                                                                                                                                // </div>
