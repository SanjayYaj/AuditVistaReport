        ...
        <React.Fragment>
            <Col lg={4} md={4} >
                <div className="col-12">
                    <input type="search" className="form-control" id="search-bar-0" value={value || ""} placeholder={`Search Users`} onChange={e => { setValue(e.target.value); onChange(e.target.value) }} />
                </div>
            </Col>
    ...
    return (
        <Fragment>
            <Row className="">
                ...
                {dynamicBtn && canEdit && (
                    <Col lg={data.length === 0 ? 8 : 2} md={data.length === 0 ? 8 : 2} >
                    ...
                    <div className="d-flex align-items-center justify-content-between">
                        <div className="d-flex">
...

                            {
                                ...
                                <div className="mb-1">
                                    {
                                        !labelDefault  && selectedEOPT.length > 0  &&
                                        
                                        <button className="btn btn-sm btn-outline-danger mx-2" onClick={() => { removeFromLabel(); }} >
                                            <i className="mdi mdi-account-group  font-size-12 me-2" />
                                            Ungroup selected
                                        </button>
                                    }

                                    {
                                        ...
                                        <button className="btn btn-sm btn-outline-danger" onClick={() => { confirmDelete() }} >
                                            <i className="mdi mdi-account-group  font-size-12 me-2" />Delete this group
                                        </button>
                                    }
                        ...
                        <div className="d-flex align-items-center justify-content-end">
                            <div className="button-items ms-2 text-center">
                                <div className="btn-group btn-sm" role="group" aria-label="Basic radio toggle button group" >
                                    <Button color={"dark"} className="btn-sm" outline onClick={() => filterStatus("All")} >
                                        {" "} Total users&nbsp; {totalEntities}{" "}
                                    </Button>

                                    {resultData.map(
                                        (itemStatus, idx) => {
                                            return (
                                                <Button key={idx} color={itemStatus.badgeClass} className="btn-sm" outline={itemStatus.status !== show_selected} onClick={() => filterStatus(itemStatus.status)} >
                                                    {" "} {itemStatus.status}{" "} &nbsp;&nbsp;&nbsp; {itemStatus.count}{" "}
                                                </Button>
                                            );
                                        }
                                    )}
