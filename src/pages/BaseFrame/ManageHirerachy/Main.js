...
import { useHierarchyValidation } from 'utils/useHierarchyValidation';
const Main = () => {
  ...
  const { canView, canEdit } = usePermissions("hirchy");
    const { debouncedHierarchyValidation } = useHierarchyValidation(sethierarchyNameExist, 500);
  ...
  const handleValidHrchySubmit = async (e, values) => {
    if (isProcessing.current) return;
    ...
    if (!isEdit) {
      await debouncedHierarchyValidation(hInfo.hname, false);
      
      setTimeout(() => {
        if (hierarchyNameExist === false) {
          createNewHirerachy(hInfo);
          setmodal(false);
        } else {
 ...
 const handleHierarchyNameChange = (e) => {
    const value = e.target.value;
    setHname(value);
    sethierarchyNameExist(false);
    debouncedHierarchyValidation(value, isEdit); 
  };
  ...
  // }
   ...
   const validateHierarchyName = (event) => {
    handleHierarchyNameChange(event);
  }
                      ...
                      <Col className="col-12">
                        <div className="mb-3">
                          ...
                          <AvField
                            ...
                            onChange={validateHierarchyName}
