...
const MapUserInfo = () => {
            ...
            checkboxes.forEach(checkbox => {
                if (checkbox.id === 'selectAll') {
                    checkbox.checked =
                        ...
                        statusBasedFilteredData.every(user =>
                            selectedUsers.some(selected => selected._id === user._id)
                        );
                }
            });
        }
    }, [selectedUsers, statusBasedFilteredData, dataLoaded]);
    ...
    // }
     ...
     const getUpdateChild = async (userNodeInfo, db_info) => {
        ...
        try {
            ...
            Swal.fire({
                title: 'Success!',
                text: 'Users have been mapped successfully',
                icon: 'success',
                timer: 1500,
            ...
            Swal.fire({
                title: 'Error',
                text: 'Failed to map users. Please try again.',
                icon: 'error',
    ...
    // }
     ...
     const mapSelectedUser = async () => {
        // Show confirmation before mapping users
        if (selectedUsers.length === 0) {
            Swal.fire({
                title: 'No Users Selected',
                text: 'Please select at least one user to map',
                icon: 'warning',
                confirmButtonText: 'OK'
            });
            return;
        ...
        const result = await Swal.fire({
            title: 'Map Selected Users',
            text: `Do you want to map ${selectedUsers.length} selected users?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonText: 'Yes, Map Users',
        ...
        if (result.isConfirmed) {
            ...
            var unique_userpath = await uniqueByKeys(userNodeInfo.nodeInfo.node.user_path, ['_id'])
            userNodeInfo["nodeInfo"]["node"]["user_path"] = unique_userpath
            ...
            if (clientInfo.allowFollowup === true) {
                ...
                // Success message will be shown by getUpdateChild function
            }
            else {
                addUsers(userNodeInfo)
    ...
    const handleSelectAll = (selected, rows) => {
        console.log("handleSelectAll", selected, rows.length);

        if (selected) {
            const newSelectedUsers = [...selectedUsers];
            const nodeInfoData = _.cloneDeep(nodeInfo);

            rows.forEach(user => {
                if (!newSelectedUsers.some(u => u._id === user._id)) {
                    const userInfo = {
                        email_id: user.email_id || "",
                        phone_num: (user.countrycode || "") + (user.phone_number || ""),
                        // cat_type: [],
                        // hirerachy_name: [],
                    ...
                    newSelectedUsers.push(userInfo);

                    if (!nodeInfoData.node.unique_users.some(u => u._id === user._id)) {
                        nodeInfoData.node.unique_users.push(userInfo);
                    }
                }
            });
            ...
            const visibleIds = rows.map(user => user._id);
            const filteredUsers = selectedUsers.filter(user => !visibleIds.includes(user._id));

            const nodeInfoData = _.cloneDeep(nodeInfo);
            nodeInfoData.node.unique_users = nodeInfoData.node.unique_users.filter(
                user => !visibleIds.includes(user._id)
            );
    ...
    //         nodeInfoData.node.user_permission_acpln = nodeInfoData.node.user_permission_acpln.filter(
    //             user => user._id !== userInfo.user_id
    //         );

    //         const filteredSelectedUsers = selectedUsersInfo.filter(
    //             user => user._id !== userInfo._id
    //         );

    //         selectedUsersInfo.length = 0;
    //         selectedUsersInfo.push(...filteredSelectedUsers);
    //     }
...
const onSelectUsers = (userInfo, event) => {
        ...
        if (event.target.checked) {
            ...
            // Swal.fire({
            //     title: 'User Selected',
            //     text: `${userInfo.fullname || userInfo.firstname} has been selected`,
            //     icon: 'success',
            //     timer: 1500,
            //     showConfirmButton: false
            // });
            ...
            // Show SweetAlert for deselection
            // Swal.fire({
            //     title: 'User Deselected',
            //     text: `${userInfo.fullname || userInfo.firstname} has been removed`,
            //     icon: 'info',
            //     timer: 1500,
            //     showConfirmButton: false
            // });
...
const onSelectValues = async (event, data, index) => {
        ...
        var unique_userpath = await uniqueByKeys(nodeInfo.node.user_path, ['_id'])
    ...
    const columns = useMemo(() => [
            ...
            Header: () => (
                <div className="form-check">
                    <input
                        ...
                        id="selectAll"
                        onChange={e => handleSelectAll(e.target.checked, statusBasedFilteredData)}
                    />
                    <label className="form-check-label" htmlFor="selectAll">All</label>
                </div>
            ),
            Cell: (cellProps) => {
        ...
        },
        {
            ...
            Cell: (cellProps) => {
                ...
                const hasDepartments = Array.isArray(item.dept_id) && item.dept_id.length > 0;

                const deptNames = hasDepartments
                    ? item.dept_id.map(dept => dept.dept_name).join(', ')
                    : ' ';
        ...
        },
        {
            ...
            Cell: (cellProps) => {
                ...
                const deptName = item.designation ? item.designation[0]?.desgn_name : 'N/A';
    ...
    ], [nodeInfo, userViewData, selectedUsers, statusBasedFilteredData, addedUsers]);
