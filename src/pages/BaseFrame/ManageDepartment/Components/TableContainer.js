...
function GlobalFilter({
  ...
  return (
    <React.Fragment>
      <Col xxl={3} lg={6}>
        <input type="search" className="form-control" style={{borderRadius:25}} id="search-bar-0" value={value || ""} placeholder={'Search...'} onChange={e => { setValue(e.target.value); onChange(e.target.value) }} />
      </Col>
...

        {
          ...
          <Col className="d-flex align-items-center justify-content-end">
            <div className="d-flex align-items-center justify-content-end gap-1">
              <Button className="btn btn-sm " color="buttonPrimaryE" onClick={() => { handleReset() }}>
                <i className="bx bx-reset" style={{ fontSize: '13px', lineHeight:1.1 }} />
              </Button>
              <Button onClick={() => { handleNewBranch() }} className="btn btn-sm  px-3" color="buttonPrimaryE" type="button">
                 {id === 'dept' ? 'Add New Department' : 'Add New Designation'}
              </Button>
            </div>
          </Col>
